...
function detectFormat(content: string): 'csv' | 'ofx' | 'qif' | 'mt940' | 'unknown' {
  const trimmed = content.trim();
  if (!trimmed) return 'unknown';
  // OFX: starts with OFXHEADER or <OFX>
  if (/^(OFXHEADER:|<OFX>)/i.test(trimmed)) return 'ofx';
  // QIF: starts with !Type:
  if (trimmed.startsWith('!Type:')) return 'qif';
  // MT940: starts with :20: or :25:
  if (/^(\{|:20:|:25:)/.test(trimmed)) return 'mt940';
...
function parseAmount(input: string): number {
  ...
  if (!cleaned) return 0;
  // CR/DR suffix
  if (/cr$/i.test(cleaned)) return Math.abs(Number(cleaned.replace(/cr$/i, ''))) || 0;
  if (/dr$/i.test(cleaned)) return -Math.abs(Number(cleaned.replace(/dr$/i, ''))) || 0;
...
export const defaultMultiformatParser: MultiformatParser = {
  detectFormat,
