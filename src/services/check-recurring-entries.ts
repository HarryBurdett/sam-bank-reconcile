...
import { getRecurringEntriesMode, type RecurringEntriesMode } from './settings.js';
...
function outstandingDates(
  ...
  while (current.getTime() <= todayMs && dates.length < maxRemaining) {
    ...
    if (++safety > 240) break; // hard safety
  }
  return dates;
}

function ledgerTypeForAeType(aeType: number): PostingLedgerType {
 ...
 */
async function loadAccountDescriptions(
  operaDb: Knex,
  nominalCodes: Set<string>,
  customerCodes: Set<string>,
  supplierCodes: Set<string>,
): Promise<Map<string, string>> {
  const out = new Map<string, string>();
  const fetchInto = async (
    table: 'nacnt' | 'sname' | 'pname',
    codeCol: string,
    descCol: string,
    codes: Set<string>,
  ): Promise<void> => {
    ...
    try {
      ...
      const rows = (await operaDb(table)
        .select(
          operaDb.raw(`RTRIM(${codeCol}) AS code`),
          operaDb.raw(`RTRIM(${descCol}) AS description`),
        )
        .whereRaw(`RTRIM(${codeCol}) IN (${placeholders})`, list)) as Array<{
        code: string | null;
 ...
 * made any decisions.
 */
export async function checkRecurringEntries(
  operaDb: Knex,
  appDb: Knex | null,
  bankCode: string,
  ...
  try {
    rows = (await operaDb.raw(
      ...
      [bc, today],
    )) as unknown as HeaderLineJoinRow[];
  } catch (err: any) {
  ...
  const nominalCodes = new Set<string>();
  ...
  for (const r of rows) {
    ...
    if (t === 3 || t === 4) customerCodes.add(acct);
    else if (t === 5 || t === 6) supplierCodes.add(acct);
  }
  const accountDescs = await loadAccountDescriptions(
    operaDb,
    nominalCodes,
    customerCodes,
    supplierCodes,
  ...
  const grouped = new Map<string, { header: HeaderLineJoinRow; lines: HeaderLineJoinRow[] }>();
  for (const r of rows) {
  ...
  const entries: RecurringEntryItem[] = [];
  let postableCount = 0;
  let blockedCount = 0;
  ...
  for (const [entryRef, group] of grouped) {
    ...
    const totalAmountPence = lines.reduce(
      (acc, l) => acc + Math.abs(Number(l.at_value ?? 0)),
      0,
    );
    const totalAmountPounds =
      Math.round((totalAmountPence / 100) * 100) / 100; // round to 2dp
    ...
    const lineDetails: RecurringEntryLineDetail[] = lines.map((l) => {
    ...
    const outstanding = nxt
      ...
      : [];
    const dateList: (string | null)[] = outstanding.length > 0 ? outstanding : [nxt];
    ...
    for (const postDate of dateList) {
      ...
      if (![1, 2, 3, 4, 5, 6].includes(aeType)) {
        ...
        try {
          const decision: PeriodPostingDecision = await getPeriodPostingDecision(
            operaDb,
            postDate,
            ledgerTypeForAeType(aeType),
          );
          if (!decision.canPost) {
            canPost = false;
