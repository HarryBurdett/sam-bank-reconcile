...
const BANK_STATEMENT_PATTERNS: Record<string, BankPattern> = {
  barclays: {
...
const BANK_STATEMENT_EXTENSIONS = new Set<string>([
  '.csv', '.ofx', '.qif', '.mt940', '.sta', '.pdf',
]);

const BANK_STATEMENT_CONTENT_TYPES = new Set<string>([
  'text/csv',
  'application/csv',
  'text/plain',
...
export function detectBankFromEmail(
  ...
  for (const [bankName, patterns] of Object.entries(BANK_STATEMENT_PATTERNS)) {
    for (const p of patterns.sender_patterns) {
      if (fromLower.includes(p.toLowerCase())) return bankName;
    }
    for (const p of patterns.filename_patterns) {
      if (filenameLower.includes(p.toLowerCase())) return bankName;
    }
    for (const p of patterns.filename_patterns) {
      if (subjectLower.includes(p.toLowerCase())) return bankName;
    }
  }
  return null;
}
...
export function extractStatementNumberFromFilename(
  filename: string | null | undefined,
  ...
  const formatDay = (n: number): string => `${n}`.padStart(2, '0');
  const monthAbbr = (n: number): string => MONTH_ABBRS[n - 1] ?? '???';
...
export function isBankStatementAttachment(
  input: IsBankStatementInput,
): boolean {
  ...
  if (!BANK_STATEMENT_EXTENSIONS.has(ext)) {
    ...
    if (!ct || !BANK_STATEMENT_CONTENT_TYPES.has(ct)) return false;
  }
  ...
  const hasBankInFilename = KNOWN_BANK_KEYWORDS.some((b) =>
    filenameLower.includes(b),
  );
  ...
  if (hasBankInFilename) return true;
...
export function compareSortKeys(a: StatementSortKey, b: StatementSortKey): number {
  for (let i = 0; i < 4; i++) {
    const av = a[i] ?? 0;
    const bv = b[i] ?? 0;
