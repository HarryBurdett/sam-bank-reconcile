...
export async function scanAllBanksFaithful(
  ...
  const extractor: PdfExtractor | null = opts.extractor ?? null;
  const emailAttachments: EmailAttachmentProvider | null =
    opts.emailAttachments ?? null;
  const t0 = Date.now();
        ...
        for (const filename of files) {
          ...
          if (cachedInfo && validateBalances) {
            ...
            if (extracted) {
              ...
              (stmt as unknown as Record<string, unknown>).transaction_count =
                extracted.transaction_count;
              (stmt as unknown as Record<string, unknown>).max_transaction_date =
  ...
  // GAP-3.
  const extractStatementPeriodKey = (fn: string): string | null => {
    const lower = fn.toLowerCase();
    // Monzo: Monzo_bank_statement_2026-01-01-2026-01-31_4539.pdf
    const monzo = lower.match(/(\d{4}-\d{2}-\d{2})[_-](\d{4}-\d{2}-\d{2})/);
    if (monzo) return `${monzo[1]}_${monzo[2]}`;
    // Barclays: Statement DD-MMM-YY AC XXXXXXXX XXXXXXXX.pdf
    const barclays = lower.match(
      /statement\s+(\d{1,2}-[a-z]{3}-\d{2})\s+ac\s+(\d{8})/,
    );
    if (barclays) return `${barclays[1]}_${barclays[2]}`;
    return null;
  };
  const extractPeriodDates = (
    fn: string,
  ...
  for (const bank of Object.values(allBanks)) {
    ...
    const byPeriod = new Map<string, StatementCandidate>();
    const stage2: StatementCandidate[] = [];
    for (const stmt of stage1) {
      ...
      if (existingHasSuffix && !stmtHasSuffix) {
        ...
        stage2.push(stmt);
        logger.info(
          `period-dedup[${bank.bank_code}]: kept ${stmt.filename} (bare-filename), dropped ${existing.filename}`,
        );
      } else if (!existingHasSuffix && stmtHasSuffix) {
        // Existing is the bare original — keep it, drop the suffixed copy.
        logger.info(
          `period-dedup[${bank.bank_code}]: dropped duplicate ${stmt.filename} (suffixed copy of ${existing.filename})`,
        );
      } else {
        ...
        stage2.push(stmt);
        logger.info(
          `period-dedup[${bank.bank_code}]: keeping both ${existing.filename} and ${stmt.filename} for content-based dedup post-extraction`,
        );
      }
    }
    ...
    const byStart = new Map<string, StatementCandidate>();
    const stage3: StatementCandidate[] = [];
    for (const stmt of stage2) {
      ...
      if (dates.end > existingDates.end) {
        ...
        if (idx >= 0) stage3.splice(idx, 1);
        byStart.set(dates.start, stmt);
        stage3.push(stmt);
        logger.info(
          `start-date-dedup[${bank.bank_code}]: ${stmt.filename} (${dates.end}) supersedes ${existing.filename} (${existingDates.end})`,
        );
      } else if (dates.end < existingDates.end) {
        logger.info(
          `start-date-dedup[${bank.bank_code}]: ${stmt.filename} (${dates.end}) superseded by ${existing.filename} (${existingDates.end})`,
        );
      } else {
        stage3.push(stmt);
      }
        ...
        try {
          ...
          if (
            ...
            const txns = Array.isArray(extracted.transactions)
              ...
              : [];
            (target as unknown as Record<string, unknown>).transaction_count =
              txns.length;
            let maxDate = '';
            for (const t of txns) {
              const d = (t.date ?? '').toString().slice(0, 10);
            ...
            (target as unknown as Record<string, unknown>).max_transaction_date =
              maxDate || null;
            (target as unknown as Record<string, unknown>).extraction_status =
              'extracted';
            (target as unknown as Record<string, unknown>).extraction_error =
  ...
  for (const bank of Object.values(banksWithStatements)) {
    const byPeriodKey = new Map<string, StatementCandidate[]>();
    for (const s of bank.statements) {
      if (!s.period_start || !s.period_end) continue;
      const key = `${s.period_start}|${s.period_end}`;
      const group = byPeriodKey.get(key);
      if (group) group.push(s);
      else byPeriodKey.set(key, [s]);
    }
    const droppedFilenames = new Set<string>();
    for (const [key, group] of byPeriodKey) {
      if (group.length < 2) continue;
      // Pick the winner by content.
      const ranked = group.slice().sort((a, b) => {
        const aCount = Number(
          (a as unknown as Record<string, unknown>).transaction_count ?? 0,
        );
        const bCount = Number(
          (b as unknown as Record<string, unknown>).transaction_count ?? 0,
        );
        if (aCount !== bCount) return bCount - aCount;
        const aMax = String(
          (a as unknown as Record<string, unknown>).max_transaction_date ?? '',
        );
        const bMax = String(
          (b as unknown as Record<string, unknown>).max_transaction_date ?? '',
        );
      ...
      for (const loser of ranked.slice(1)) {
        ...
        logger.info(
          `content-dedup[${bank.bank_code}]: period ${key} — kept ${winner.filename} ` +
            `(txns=${(winner as unknown as Record<string, unknown>).transaction_count ?? '?'}, ` +
            `max_date=${(winner as unknown as Record<string, unknown>).max_transaction_date ?? '?'}), ` +
            `dropped ${loser.filename} ` +
            `(txns=${(loser as unknown as Record<string, unknown>).transaction_count ?? '?'}, ` +
            `max_date=${(loser as unknown as Record<string, unknown>).max_transaction_date ?? '?'})`,
        );
      }
    }
    if (droppedFilenames.size > 0) {
      bank.statements = bank.statements.filter(
        (s) => !droppedFilenames.has(s.filename),
      );
      bank.statement_count = bank.statements.length;
    }
  }
  ...
  for (const bank of Object.values(banksWithStatements)) {
    const bankRecOpenings =
      tracking.reconciled_opening_balances.get(bank.bank_code) ??
      new Set<number>();
    const effectiveRec =
      typeof bank.reconciled_balance === 'number'
        ? bank.reconciled_balance
        : null;
    const remaining: StatementCandidate[] = [];
    for (const stmt of bank.statements) {
      ...
      if (result.chainComplete) {
        ...
        nonCurrent.already_processed.push(stmt);
        logger.info(
          `chain-check[${bank.bank_code}]: ${stmt.filename} → already_processed (${result.reasonKind})`,
        );
      } else {
        remaining.push(stmt);
      }
