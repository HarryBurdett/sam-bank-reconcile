...
export type SolverCandidateSource =
  | 'labelled'
  | 'summary'
  | 'chain_forward_post_txn'
  | 'chain_forward_pre_txn'
  | 'chain_backward'
  | 'external_reconciled';
 ...
 */
function sumAmounts(txns: SolverTxn[]): number | null {
 ...
 */
function deriveChainForwardCandidates(
  txns: SolverTxn[],
): { postTxn: SolverCandidate | null; preTxn: SolverCandidate | null } {
  const withBoth = txns.filter(
    (t) => nonNull(t.amount) && nonNull(t.balance),
  );
  if (withBoth.length === 0) {
    return { postTxn: null, preTxn: null };
  }
  let bestPost: SolverCandidate | null = null;
  let bestPre: SolverCandidate | null = null;
  for (const candidate of withBoth) {
    ...
    if (!bestPost || walkPost.chained.length > bestPost.chainedTxns.length) {
      bestPost = {
        ...
        derivedClosing:
          walkPost.chained.length === withBoth.length
            ? walkPost.terminal
            : null,
      };
    }
    ...
    if (!bestPre || walkPre.chained.length > bestPre.chainedTxns.length) {
      bestPre = {
        ...
        derivedClosing:
          walkPre.chained.length === withBoth.length
            ? walkPre.terminal
            : null,
      };
    }
...
export function solveStatementBalance(input: SolverInput): SolverResult {
  const notes: string[] = [];
  ...
  if (nonNull(input.labelledOpening)) {
    ...
    const best = walks.reduce((a, b) =>
      a.chained.length >= b.chained.length ? a : b,
    );
    ...
    notes.push(
      `labelled opening ${input.labelledOpening.toFixed(2)} chains ${best.chained.length}/${input.txns.length} (${postTxnUsed ? 'post' : 'pre'}-txn semantics)`,
    );
  }
  ...
  if (
    ...
    const opening = round2(
      input.labelledClosing - (input.summaryTotalIn - input.summaryTotalOut),
    );
    const walks = [
    ...
    notes.push(
      `summary arithmetic opening ${opening.toFixed(2)} chains ${best.chained.length}/${input.txns.length}`,
    );
  }
  ...
  if (chainCands.postTxn) {
    ...
    notes.push(
      `forward chain (post-txn) opening ${chainCands.postTxn.opening.toFixed(2)} chains ${chainCands.postTxn.chainedTxns.length}/${input.txns.length}`,
    );
  }
  if (chainCands.preTxn) {
    candidates.push(chainCands.preTxn);
    notes.push(
      `forward chain (pre-txn) opening ${chainCands.preTxn.opening.toFixed(2)} chains ${chainCands.preTxn.chainedTxns.length}/${input.txns.length}`,
    );
  }
  ...
  if (nonNull(input.labelledClosing)) {
    ...
    if (total !== null) {
      ...
      notes.push(
        `backward chain opening ${opening.toFixed(2)} chains ${best.chained.length}/${input.txns.length}`,
      );
    }
  }
  ...
  if (nonNull(input.externalReconciledBalance)) {
    ...
    notes.push(
      `external reconciled ${input.externalReconciledBalance.toFixed(2)} chains ${best.chained.length}/${input.txns.length}`,
    );
  }
  ...
  const winningCand = winner.cands.reduce(
    (a, b) => (a.chainedTxns.length >= b.chainedTxns.length ? a : b),
    winner.cands[0]!,
  );
  ...
  if (conflictGroups.length >= 2) {
    return {
      ok: false,
      reason:
        `Disagreement: multiple candidate openings each backed by ≥2 facts: ` +
        conflictGroups
          .map(
            (g) => `£${g.value.toFixed(2)} (${g.cands.map((c) => c.source).join(',')})`,
          )
          .join('; '),
      candidates,
      notes,
  ...
  if (nonNull(input.labelledClosing)) {
    if (winningCand.derivedClosing !== null) {
      if (!nearlyEqual(winningCand.derivedClosing, input.labelledClosing)) {
        ...
        // labelled closing.
        notes.push(
          `chain terminal ${winningCand.derivedClosing.toFixed(2)} differs from labelled closing ${input.labelledClosing.toFixed(2)} — trusting label`,
        );
      }
    }
