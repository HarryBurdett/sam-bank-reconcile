...
export async function completeBatch(
  ...
  try {
    return await withImportLock(
      ...
      async () => {
        // 1. Validate aentry
        const aentryRows = (await operaDb.raw(
          `SELECT ae_entry, ae_acnt, ae_complet, ae_value, ae_lstdate, ae_cbtype
           FROM aentry WITH (NOLOCK)
           WHERE ae_entry = ? AND RTRIM(ae_acnt) = ?`,
          [entryNumber, bankCode],
        )) as unknown as Array<{
          ae_entry: string;
          ae_acnt: string;
          ae_complet: number;
          ae_value: number;
          ae_lstdate: Date | string;
          ae_cbtype: string;
        }>;
        ...
        const postDate =
          entry.ae_lstdate instanceof Date
        ...
        const atranRows = (await operaDb.raw(
          `SELECT at_unique FROM atran WITH (NOLOCK)
           WHERE ae_entry = ? AND RTRIM(ae_acnt) = ?`,
          [entryNumber, bankCode],
        )) as unknown as Array<{ at_unique: string }>;
        ...
        const anomlRows = (await operaDb.raw(
          ...
          ax_nacnt: string;
          ax_source: string | null;
          ax_date: Date | string;
          ax_value: number;
          ax_tref: string | null;
          ax_comment: string | null;
          ax_unique: string;
        }>;
            ...
            await trx.raw(
              `UPDATE anoml WITH (ROWLOCK)
               ...
               WHERE RTRIM(ax_unique) = ?
                 AND RTRIM(ax_nacnt) = ?
                 AND ax_done = 'N'`,
              [nextJournal, axUnique, nacntCode],
            );
          }
