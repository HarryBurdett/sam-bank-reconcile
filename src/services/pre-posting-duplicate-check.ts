 ...
 */
async function typeBlindAtranMatch(
  operaDb: Knex,
  bankCode: string,
  transactionDate: string,
  signedAmountPounds: number,
  excludeEntryNumbers: Iterable<string> = [],
  dateToleranceDays = 7,
): Promise<{ entryNumber: string | null; atType: number | null; postedDate: string | null }> {
  const signedPence = Math.round(signedAmountPounds * 100);
  if (signedPence === 0) return { entryNumber: null, atType: null, postedDate: null };
  ...
  let query = `
    ...
    WHERE t.at_acnt = ?
      ...
      AND a.ae_reclnum = 0
      AND a.ae_remove = 0`;
  const bindings: Array<string | number> = [bankCode, signedPence, dateFrom, dateTo];
...
export async function checkTypeBlindAtranMatch(args: {
  ...
  const hit = await typeBlindAtranMatch(
    args.operaDb,
    args.bankCode,
    args.transactionDate,
    args.signedAmountPounds,
    args.excludeEntryNumbers,
    args.dateToleranceDays ?? 7,
  );
  if (hit.entryNumber) {
    return {
      isDuplicate: true,
      ...
      reason:
        `Already in Opera as ${hit.entryNumber} ` +
...
export async function checkCashbookDuplicateBeforePosting(
  ...
  const blind = await typeBlindAtranMatch(
    operaDb,
    bankCode,
    transactionDate,
    signedAmountPounds,
  ...
  if (blind.entryNumber) {
    return {
      ...
      reason:
        `Already in Opera as ${blind.entryNumber} ` +
