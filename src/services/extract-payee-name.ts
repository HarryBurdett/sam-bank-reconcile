...
const BANK_METHOD_SUFFIX_RE =
  /\s*\(\s*(?:faster\s*pay(?:\.\.\.|…|ments?)|direct\s*debit|standing\s*order|bacs|chaps|card\s*payment|cheque|cash|online\s*payment|transfer)\s*\)\s*$/i;
...
const FALLBACK_PREFIX_RE =
  /^(?:dd\s+)?(?:card\s+payment|direct\s+debit|direct\s+credit|faster\s+payment|standing\s+order|(?:giro\s+)?(?:bank\s+)?giro\s+credit|counter\s+credit)\s*/i;
...
export function extractPayeeName(description: string | null | undefined): string {
  ...
  if (text.includes(',')) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (
        CLASSIFICATION_WORDS.has(upper) ||
