...
function getAllMatchNames(c: MatchCandidate): Array<{ name: string; source: string }> {
  const names: Array<{ name: string; source: string }> = [];
  if (c.primary_name) names.push({ name: c.primary_name, source: 'primary' });
  if (c.payee_name?.trim()) names.push({ name: c.payee_name, source: 'payee' });
  ...
  if (Array.isArray(c.search_keys)) {
    c.search_keys.forEach((key, i) => {
      if (key?.trim()) names.push({ name: key, source: `key${i + 1}` });
    });
  }
  return names;
}
...
function getSignificantTokens(name: string): Set<string> {
...
function wordContainmentScore(name: string, candidate: string): number {
  if (!name || !candidate) return 0;
  const nameT = getSignificantTokens(name);
  const candT = getSignificantTokens(candidate);
  if (nameT.size === 0) return 0;
  let matches = 0;
  for (const t of nameT) if (candT.has(t)) matches += 1;
  return matches / nameT.size;
}

function prefixMatchScore(name: string, candidate: string): number {
  if (!name || !candidate) return 0;
...
export function calculateMatchScore(
  ...
  if (prefScore > 0.5) combined = Math.max(combined, prefScore);
  if (contScore >= 0.9 && tokScore >= 0.5) combined = Math.max(combined, 0.85);
...
function matchAgainst(
  ...
  if (!name || candidates.size === 0) return { ...EMPTY_MATCH };
  let best: MatchResult = { ...EMPTY_MATCH };
  for (const [account, candidate] of candidates) {
    for (const { name: candName, source } of getAllMatchNames(candidate)) {
      ...
      if (source !== 'primary' && score > 0) score *= 0.95;
      if (score > best.score) {
        best = {
          account,
...
export class BankMatcher {
  ...
  readonly customers = new Map<string, MatchCandidate>();
  readonly suppliers = new Map<string, MatchCandidate>();

  constructor(minScore: number = 0.6) {
    this.minScore = minScore;
  }

  loadCustomers(customers: MatchCandidate[]): void {
    this.customers.clear();
    for (const c of customers) this.customers.set(c.account, c);
  }

  loadSuppliers(suppliers: MatchCandidate[]): void {
    this.suppliers.clear();
    for (const s of suppliers) this.suppliers.set(s.account, s);
  }

  matchCustomer(name: string): MatchResult & { is_match: boolean } {
  ...
  matchSupplier(name: string): MatchResult & { is_match: boolean } {
 ...
 */
export async function loadCustomerCandidates(
  ...
  try {
    const rows = (await operaDb('sname')
      .select(
        ...
        operaDb.raw('RTRIM(sn_name) as primary_name'),
        operaDb.raw("RTRIM(ISNULL(sn_key1, '')) as key1"),
        operaDb.raw("RTRIM(ISNULL(sn_key2, '')) as key2"),
        operaDb.raw("RTRIM(ISNULL(sn_key3, '')) as key3"),
        operaDb.raw("RTRIM(ISNULL(sn_key4, '')) as key4"),
        operaDb.raw("RTRIM(ISNULL(sn_vendor, '')) as vendor_ref"),
      )
      .where(function notStopped(this: import('knex').Knex.QueryBuilder) {
        this.where('sn_stop', 0).orWhereNull('sn_stop');
      })
      .andWhere(function notDormant(this: import('knex').Knex.QueryBuilder) {
  ...
  try {
    const rows = (await operaDb('pname')
      .select(
        ...
        operaDb.raw('RTRIM(pn_name) as primary_name'),
        operaDb.raw("RTRIM(ISNULL(pn_payee, '')) as payee_name"),
        operaDb.raw("RTRIM(ISNULL(pn_key1, '')) as key1"),
        operaDb.raw("RTRIM(ISNULL(pn_key2, '')) as key2"),
        operaDb.raw("RTRIM(ISNULL(pn_key3, '')) as key3"),
        operaDb.raw("RTRIM(ISNULL(pn_key4, '')) as key4"),
      )
      .where(function notStopped(this: import('knex').Knex.QueryBuilder) {
