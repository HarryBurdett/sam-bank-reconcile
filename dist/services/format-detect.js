 ...
 */
const SUPPORTED_FORMATS = ['CSV', 'OFX', 'QIF', 'MT940'];
...
const ofxSniffer = {
    ...
    canParse(content, filename) {
        ...
        return (start.startsWith('OFXHEADER:') ||
            start.includes('<?OFX') ||
            start.includes('<OFX>'));
    },
};
...
const mt940Sniffer = {
    ...
    canParse(content, filename) {
        const lower = filename.toLowerCase();
        if (lower.endsWith('.mt940') ||
            lower.endsWith('.sta') ||
            lower.endsWith('.940')) {
            return true;
        }
        const start = content.slice(0, 200).trim();
        return (start.startsWith(':20:') ||
            start.startsWith('{1:') ||
            start.includes(':60F:') ||
            start.includes(':61:'));
    },
...
const SNIFFERS = [csvSniffer, ofxSniffer, qifSniffer, mt940Sniffer];
/**
...
export function detectFormat(content, filename = '') {
    for (const sniffer of SNIFFERS) {
        if (sniffer.canParse(content, filename))
            return sniffer.format;
    }
    return null;
}
//# sourceMappingURL=format-detect.js.map