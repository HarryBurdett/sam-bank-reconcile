...
export async function reconcileBank(operaDb, bankCode, now = new Date()) {
    ...
    try {
        ...
        const bankInfo = (await operaDb('nbank')
            ...
            .select('nk_acnt', operaDb.raw('RTRIM(nk_desc) AS description'), 'nk_sort', 'nk_number')
            .first());
        if (!bankInfo) {
        ...
        const cyRow = (await operaDb('ntran')
            ...
            .first());
        const currentYear = cyRow?.current_year !== null && cyRow?.current_year !== undefined
        ...
        const cbCyRow = (await operaDb('atran')
            ...
            .select(operaDb.raw('COUNT(DISTINCT at_entry) AS entry_count'), operaDb.raw('COUNT(*) AS transaction_count'), operaDb.raw('SUM(CASE WHEN at_value > 0 THEN at_value ELSE 0 END) AS receipts_pence'), operaDb.raw('SUM(CASE WHEN at_value < 0 THEN ABS(at_value) ELSE 0 END) AS payments_pence'), operaDb.raw('SUM(at_value) AS net_pence'))
            .first());
        ...
        const cbAllRow = (await operaDb('atran')
            .where({ at_acnt: code })
            .select(operaDb.raw('COUNT(DISTINCT at_entry) AS entry_count'), operaDb.raw('COUNT(*) AS transaction_count'), operaDb.raw('SUM(at_value) AS net_pence'))
            .first());
        ...
        const nacntRow = (await operaDb('nacnt')
            .where({ na_acnt: code })
            .select('na_acnt', operaDb.raw('RTRIM(na_desc) AS description'), 'na_ytddr', 'na_ytdcr', 'na_prydr', 'na_prycr')
            .first());
        let bfBalance = 0;
        let nlTotal = 0;
        let nlDetails;
        if (nacntRow) {
            ...
            const ntranRow = (await operaDb('ntran')
                ...
                .select(operaDb.raw('SUM(CASE WHEN nt_value > 0 THEN nt_value ELSE 0 END) AS debits'), operaDb.raw('SUM(CASE WHEN nt_value < 0 THEN ABS(nt_value) ELSE 0 END) AS credits'), operaDb.raw('SUM(nt_value) AS net'))
                .first());
        ...
        try {
            anomlPending = (await operaDb('anoml')
                ...
                .select('ax_nacnt', 'ax_source', 'ax_date', 'ax_value', 'ax_tref', 'ax_comment', 'ax_done'));
        }
        catch {
            anomlPending = [];
        }
        ...
        try {
            anomlSummary = (await operaDb('anoml')
                ...
                .select(operaDb.raw("CASE WHEN ax_done = 'Y' THEN 'Posted' ELSE 'Pending' END AS status"), operaDb.raw('COUNT(*) AS count'), operaDb.raw('SUM(ax_value) AS total')));
        }
        catch {
            anomlSummary = [];
        }
        ...
        for (const row of anomlPending ?? []) {
            ...
            const dateStr = tr instanceof Date && !Number.isNaN(tr.getTime())
        ...
        const allReconciled = variance_cb_nbank_abs < 0.005 && variance_nbank_nl_abs < 0.005;
        let message;
        if (allReconciled) {
            if (pendingCount > 0) {
            ...
            if (variance_cb_nbank_abs >= 0.005) {
                issues.push(variance_cb_nbank > 0
            ...
            if (variance_nbank_nl_abs >= 0.005) {
                issues.push(variance_nbank_nl > 0
...
//# sourceMappingURL=reconcile-bank.js.map