...
import { listOrphanTmpstat, clearOrphanTmpstat, } from './services/orphan-tmpstat.js';
import { getUnreconciledEntries, getReconciliationStatus, recoverFromOperaDivergence, } from './services/reconciliation-status.js';
import { checkOrphanedTransactions, recoverOrphanedTransactions, } from './services/transaction-orphan-check.js';
import { checkRestoreAcrossAllBanks } from './services/restore-check-all.js';
import { ignoreTransaction, listIgnoredTransactions, unignoreTransactionById, unignoreTransactionByMatch, } from './services/ignored-transactions.js';
import { markStatementReconciled, listImportedStatements, } from './services/statement-files.js';
import { getRecurringEntriesMode, setRecurringEntriesMode, } from './services/settings.js';
import { listCashbookTypes } from './services/cashbook-types.js';
import { getMatchConfig, updateMatchConfig, } from './services/match-config.js';
import { detectFormat, supportedFormats, } from './services/format-detect.js';
...
import { saveImportDraft, loadImportDraft, deleteImportDraft, } from './services/bank-import-drafts.js';
import { getCustomersForDropdown, getSuppliersForDropdown, } from './services/account-dropdowns.js';
import { unreconcileEntries } from './services/unreconcile.js';
import { listImportHistory, deleteImportRecord, clearImportHistory, } from './services/import-history.js';
import { getFolderSettings, saveFolderSettings, } from './services/folder-settings.js';
import { validateStatementForReconciliation } from './services/validate-statement.js';
import { matchStatementToCashbook, } from './services/match-statement.js';
...
import { markEntriesReconciled, } from './services/mark-reconciled.js';
import { recordCorrection, listCorrections, } from './services/alias-corrections.js';
...
import { refreshMatches, } from './services/refresh-matches.js';
import { suggestAccountForTransaction, } from './services/suggest-account.js';
import { bankImportPostingExecutor } from './services/import-posting-executor.js';
import { inMemoryImportLock, makeBankStatementOverlapChecker, } from './services/import-defaults.js';
import { previewBankImportFromPdf, } from './services/preview-from-pdf.js';
import { previewBankImportFromEmail, } from './services/preview-from-email.js';
import { reconcileBankDashboard } from './services/reconcile-dashboard.js';
import { archiveFile, getArchiveHistory, restoreArchivedFile, getPendingFiles, } from './services/archive.js';
import { processStatement } from './services/process-statement.js';
import { getBankReconciliationStatus, getUnreconciledEntriesForBank, getStatementTransactionsForImport, } from './services/bank-reconciliation-status.js';
...
import { createFolderBackedFileStorage, createFolderBackedPdfContentReader, } from './services/folder-backed-storage.js';
export function createRouter(ctx) {
    ...
    const builtinFileStorage = createFolderBackedFileStorage(() => ctx.db.app);
    const builtinPdfReader = createFolderBackedPdfContentReader(() => ctx.db.app);
    const builtinPdfExtractor = ctx.llm
     ...
     */
    router.get('/api/reconcile/bank/:bank_code', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
    ...
    router.get('/api/reconcile/bank/:bank_code/status', async (req, res) => {
        ...
        try {
            ...
            const currentFilename = typeof req.query.current_filename === 'string'
                ? req.query.current_filename
                : null;
            const result = await getReconciliationStatus(operaDb, bankCode, ctx.db.app, currentFilename);
            ...
            if (ctx.db.app) {
                try {
                    const orphans = await checkOrphanedTransactions(operaDb, ctx.db.app, bankCode);
                    if (orphans.success && orphans.orphan_line_count > 0) {
                        result.orphan_transactions = {
                            ...
                            message: `${orphans.orphan_line_count} statement line(s) across ` +
     ...
     */
    router.get('/api/recurring-entries/check/:bank_code', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            const result = await checkRecurringEntries(operaDb, appDb, String(req.params.bank_code ?? ''));
            if (!result.success) {
                res.status(400).json(result);
                return;
            }
     ...
     */
    router.get('/api/bank-import/cashbook-types', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
     ...
     */
    router.get('/api/bank-import/config', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
     ...
     */
    router.put('/api/bank-import/config', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            ...
            const result = await updateMatchConfig(appDb, {
                min_match_score: q.min_match_score !== undefined ? Number(q.min_match_score) : 0.6,
                learn_threshold: q.learn_threshold !== undefined ? Number(q.learn_threshold) : 0.8,
                ambiguity_threshold: q.ambiguity_threshold !== undefined ? Number(q.ambiguity_threshold) : 0.15,
                use_phonetic: q.use_phonetic !== undefined ? q.use_phonetic === 'true' : true,
                use_levenshtein: q.use_levenshtein !== undefined ? q.use_levenshtein === 'true' : true,
     ...
     */
    router.post('/api/bank-import/detect-format', async (req, res) => {
        try {
            const body = (req.body ?? {});
     ...
     */
    router.post('/api/bank-import/detect-bank', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            ...
            if (detected.bank_code) {
                ...
                const banks = await listBanks(operaDb);
                const found = detected.sort_code && detected.account_number
     ...
     * and timestamp.
     */
    router.post('/api/bank-import/duplicate-override', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
     ...
     */
    router.post('/api/bank-import/draft', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            ...
            const result = await saveImportDraft(appDb, {
                ...
                pdfHash: body.pdf_hash ?? null,
                targetSystem: body.target_system ?? 'opera_se',
            });
            res.json(result);
        }
        catch (err) {
     ...
     */
    router.get('/api/bank-import/draft', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            ...
            const result = await loadImportDraft(appDb, {
                bankCode,
                source,
                emailId: q.email_id !== undefined && q.email_id !== ''
                    ? String(q.email_id)
                    : undefined,
                attachmentId: q.attachment_id !== undefined ? String(q.attachment_id) : undefined,
                pdfHash: q.pdf_hash !== undefined ? String(q.pdf_hash) : undefined,
     ...
     */
    router.delete('/api/bank-import/draft', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
     ...
     */
    router.post('/api/reconcile/bank/:bank_code/unreconcile', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
     ...
     */
    router.post('/api/reconcile/bank/:bank_code/mark-reconciled', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        ...
        try {
            ...
            const result = await markEntriesReconciled(appDb, operaDb, {
                ...
                closingBalance: body.closing_balance !== undefined ? Number(body.closing_balance) : null,
            });
            if (!result.success) {
                res.status(400).json(result);
                return;
     ...
     */
    router.post('/api/bank-import/correction', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
     ...
     */
    router.get('/api/bank-import/corrections', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            const result = await listCorrections(appDb, {
                bankName: typeof req.query.bank_name === 'string' ? req.query.bank_name : null,
                correctAccount: typeof req.query.correct_account === 'string'
     ...
     * the route boundary.
     */
    router.post('/api/reconcile/bank/:bank_code/complete-batch/:entry_number', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
     ...
     */
    router.post('/api/bank-import/persist-decisions', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
     ...
     *   }
     */
    router.post('/api/reconcile/bank/:bank_code/confirm-matches', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
     ...
     */
    router.get('/api/reconcile/bank/:bank_code/scan-emails', (_req, res) => {
        res.json({
            success: false,
            deprecated: true,
            redirect_to: '/api/bank-import/scan-emails',
            message: "This endpoint is deprecated — use /api/bank-import/scan-emails " +
     ...
     */
    router.get('/api/bank-import/repeat-entries', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            const result = await listRepeatEntries(operaDb, String(req.query.bank_code ?? ''));
            if (!result.success) {
                res.status(400).json(result);
                return;
            }
     ...
     */
    router.post('/api/bank-import/update-repeat-entry-date', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        ...
        try {
            ...
            const result = await updateRepeatEntryDate(appDb, operaDb, {
                ...
                statementName: typeof q.statement_name === 'string' ? q.statement_name : null,
            });
            if (!result.success) {
                res.status(400).json(result);
                return;
     ...
     */
    router.get('/api/bank-import/import-history', async (req, res) => {
        ...
        try {
            const result = await listImportHistory(appDb, {
                bankCode: typeof req.query.bank_code === 'string'
                    ? req.query.bank_code
                    : null,
                fromDate: typeof req.query.from_date === 'string'
                    ? req.query.from_date
                    : null,
                toDate: typeof req.query.to_date === 'string'
                    ? req.query.to_date
                    : null,
     ...
     * behaviour.
     */
    router.get('/api/bank-import/email-import-history', async (req, res) => {
        const appDb = ctx.db.app;
        if (!appDb) {
            res.status(503).json({
                success: false,
     ...
     */
    router.delete('/api/bank-import/import-history/:record_id', async (req, res) => {
        const appDb = ctx.db.app;
        if (!appDb) {
            res.status(503).json({
                success: false,
     ...
     * deleted count.
     */
    router.delete('/api/bank-import/import-history', async (req, res) => {
        const appDb = ctx.db.app;
        if (!appDb) {
            res.status(503).json({
                success: false,
     ...
     */
    router.get('/api/bank-import/folder-settings', async (_req, res) => {
        const appDb = ctx.db.app;
        if (!appDb) {
            res.status(503).json({
                success: false,
     ...
     * setting).
     */
    router.post('/api/bank-import/folder-settings', async (req, res) => {
        const appDb = ctx.db.app;
        if (!appDb) {
            res.status(503).json({
                success: false,
     ...
     */
    router.post('/api/bank-reconciliation/complete', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = ctx.db.app;
        ...
        try {
            ...
            if (!bankCode ||
                ...
                res.status(400).json({
                    success: false,
                    error: 'bank_code, statement_number, statement_date, closing_balance are required',
                });
                return;
            }
            const body = (req.body ?? {});
                    ...
                    if (resolvedId !== null) {
                        ...
                        try {
                            const [insertedId] = (await appDb('bank_statement_imports')
                                ...
                                .returning('id'));
                            const newId = typeof insertedId === 'number'
                                ? insertedId
                                : insertedId?.id;
                            if (newId) {
                                enriched.tracking_updated_import_id = Number(newId);
                                ctx.logger.info?.(`[reconcile-complete] created bank_statement_imports id=${newId} ` +
                            ...
                            const msg = createErr instanceof Error
                                ? createErr.message
                                : String(createErr);
                            ctx.logger.warn?.(`[reconcile-complete] could not self-create audit row: ${msg}`);
                            enriched.tracking_warning =
     ...
     *   - statement_date (required, YYYY-MM-DD)
     */
    router.post('/api/bank-reconciliation/validate-statement', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            ...
            const closingBalance = Number(req.query.closing_balance);
            const statementNumber = req.query.statement_number !== undefined &&
                req.query.statement_number !== ''
                ? Number(req.query.statement_number)
                : null;
            const statementDate = typeof req.query.statement_date === 'string'
     ...
     */
    router.post('/api/bank-reconciliation/match-statement', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
     ...
     */
    router.get('/api/bank-import/scan-emails', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const adapter = ctx;
        const mailbox = adapter.bankMailboxAdapter ?? builtinEmailIngest?.mailbox;
        if (!mailbox || !adapter.bankReconciledKeyStore) {
            res.status(503).json({
                success: false,
                error: 'Mailbox adapter or reconciled-key store not configured. SAM email-ingest wiring required.',
            });
            return;
        }
        try {
            ...
            const result = await scanEmailsForBankStatements(operaDb, appDb, mailbox, adapter.bankReconciledKeyStore, {
                bankCode,
                daysBack,
                includeProcessed,
                validateBalances,
     ...
     */
    router.post('/api/bank-import/import-from-pdf', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        ...
        if (!extractor) {
            res.status(503).json({
                success: false,
                error: 'PDF extractor not configured. SAM team must provide ctx.bankPdfExtractor or enable ctx.llm.',
            });
            return;
        }
        const executor = adapter.bankImportExecutor ?? bankImportPostingExecutor;
        const lock = adapter.bankImportLock ?? inMemoryImportLock;
        const overlapChecker = adapter.bankPeriodOverlapChecker ??
        ...
        try {
            ...
            if ((!filePath || !filePath.trim()) &&
                ...
                const { importBankStatementFromEmail } = await import('./services/bank-import-from-email.js');
                const rescued = await importBankStatementFromEmail(operaDb, appDb, adapter.bankEmailAttachments, extractor, executor, lock, overlapChecker, {
                ...
                ctx.logger.info?.(`import-from-pdf rescued to email path (email_id=${emailIdRaw}, attachment_id=${attachmentIdRaw})`);
                res.json(rescued);
                return;
            }
            const result = await importBankStatementFromPdf(operaDb, appDb, {
                filePath,
                bankCode: String(req.query.bank_code ?? body.bank_code ?? ''),
     ...
     */
    router.post('/api/bank-import/check-duplicates', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            ...
            const bankCode = req.query.bank_code ?? null;
     ...
     */
    router.get('/api/bank-import/suggest-account', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        try {
            const name = String(req.query.name ?? '');
            const transactionType = String(req.query.transaction_type ?? '');
            ...
            const result = await suggestAccountForTransaction(operaDb, name, transactionType, limit);
            res.json(result);
        }
        catch (err) {
     ...
     */
    router.post('/api/bank-import/preview-from-pdf', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
     ...
     */
    router.post('/api/bank-import/preview-from-email', async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        ...
        const attachments = ctx.bankEmailAttachments;
        if (!attachments) {
            res.status(503).json({
                success: false,
                error: 'ctx.bankEmailAttachments must be configured (standalone: ' +
        ...
        if (!llm && !extractor) {
            res.status(503).json({
                success: false,
                error: 'No PDF extractor configured. Standalone needs GEMINI_API_KEY ' +
        ...
        try {
            const body = (req.body ?? {});
            const result = await previewBankImportFromEmail(operaDb, llm, attachments, {
    ...
    // ---------------------------------------------------------------
    router.post('/api/archive/file', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const storage = ctx.fileStorage ??
            builtinFileStorage;
        if (!storage) {
            res.status(503).json({
                success: false,
                error: 'ctx.fileStorage adapter not configured. SAM team must wire a storage adapter.',
            });
            return;
        }
        try {
            const body = (req.body ?? {});
            const importType = req.query.import_type ??
                body.import_type;
    ...
    router.get('/api/archive/history', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        try {
            const importType = req.query.import_type ?? null;
    ...
    router.post('/api/archive/restore', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const storage = ctx.fileStorage ??
        ...
        try {
            const body = (req.body ?? {});
            const archivePath = String(req.query.archive_path ?? body.archive_path ?? '');
    ...
    router.get('/api/archive/pending', async (req, res) => {
        const storage = ctx.fileStorage ??
            builtinFileStorage;
        if (!storage) {
            res.status(503).json({
        ...
        try {
            const importType = String(req.query.import_type ?? '');
     ...
     */
    const handleProcessStatement = async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const llm = ctx.llm ?? null;
    ...
    router.post('/api/reconcile/process-statement-unified', handleProcessStatement);
    /**
     ...
     */
    router.post('/api/bank-import/import-with-overrides', async (req, res) => {
    ...
    router.get('/api/bank-reconciliation/statement-transactions/:import_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
    ...
    // ---------------------------------------------------------------
    router.post('/api/reconcile/bank/:bank_code/audit-defer', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const body = (req.body ?? {});
    ...
    router.get('/api/reconcile/bank/:bank_code/deferred-items', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await listDeferredItems(appDb, String(req.params.bank_code ?? '')));
    });
    router.delete('/api/reconcile/bank/:bank_code/deferred-items', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await deleteDeferredItems(appDb, String(req.params.bank_code ?? ''), Array.isArray(body.ids) ? body.ids : undefined));
    });
    router.delete('/api/reconcile/bank/ignored-transaction/:record_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await deleteIgnoredTransactionByRecordId(appDb, Number(req.params.record_id)));
    });
    // ---------------------------------------------------------------
    ...
    router.post('/api/cashbook/create-entry', async (req, res) => {
        ...
        if (!operaDb)
            return;
        const body = (req.body ?? {});
        res.json(await createCashbookEntry(operaDb, {
            bankCode: String(body.bank_code ?? ''),
            date: String(body.date ?? ''),
            amount: Number(body.amount ?? 0),
            matchedAccount: String(body.matched_account ?? ''),
            action: body.action ?? 'sales_receipt',
            reference: body.reference ?? '',
            memo: body.memo ?? '',
            cbtype: body.cbtype ?? null,
        }));
    });
    router.post('/api/cashbook/create-bank-transfer', async (req, res) => {
        ...
        if (!operaDb)
            return;
        const body = (req.body ?? {});
        res.json(await createBankTransfer(operaDb, {
    ...
    router.post('/api/cashbook/auto-match-statement-lines', async (req, res) => {
        ...
        if (!operaDb)
            return;
        const body = (req.body ?? {});
        res.json(await autoMatchStatementLines(operaDb, String(body.bank_code ?? ''), Number(body.import_id ?? 0)));
    });
    // ---------------------------------------------------------------
    ...
    router.post('/api/bank-import/archive-statement', async (req, res) => {
        ...
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await archiveStatement(appDb, Number(body.import_id ?? req.query.import_id ?? 0), String(body.archived_by ?? 'system')));
    });
    router.get('/api/bank-import/archived-statements', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await listArchivedStatements(appDb, req.query.bank_code || null, req.query.limit ? Number(req.query.limit) : 200));
    });
    router.post('/api/bank-import/restore-statement', async (req, res) => {
        ...
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await restoreStatement(appDb, Number(body.import_id ?? req.query.import_id ?? 0)));
    });
    router.get('/api/bank-import/archived-statement-pdf/:record_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        const storage = ctx.fileStorage ??
            builtinFileStorage;
        res.json(await getArchivedStatementPdf(appDb, storage, Number(req.params.record_id)));
    });
    router.post('/api/bank-import/delete-archived-statement', async (req, res) => {
        ...
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await deleteArchivedStatement(appDb, Number(body.record_id ?? req.query.record_id ?? 0)));
    });
    router.post('/api/bank-import/manage-statements', async (req, res) => {
        ...
        if (!appDb)
            return;
        const body = (req.body ?? {});
        res.json(await manageStatements(appDb, body.bank_code ?? null, !!body.include_archived));
    });
    // ---------------------------------------------------------------
    // File-list / scan-folder / scan-all-banks / pdf-content
    // ---------------------------------------------------------------
    const getFileStorage = () => ctx.fileStorage ??
        builtinFileStorage;
    const getPdfReader = () => ctx.pdfContentReader ??
        builtinPdfReader;
    const getMultiformatParser = () => ctx
        .multiformatParser ?? defaultMultiformatParser;
    const getEmailAttachments = () => ctx
    ...
    router.get('/api/bank-import/pdf-content', async (req, res) => {
        res.json(await getPdfContent(getPdfReader(), String(req.query.file_path ?? '')));
    });
    ...
    router.post('/api/bank-import/fetch-emails-to-folder', async (req, res) => {
        const body = (req.body ?? {});
        res.json(await fetchEmailsToFolder(getEmailAttachments(), getFileStorage(), Array.isArray(body.emails) ? body.emails : []));
    });
    router.get('/api/bank-import/scan-all-banks', async (req, res) => {
        ...
        const extractor = adapter.bankPdfExtractor ?? builtinPdfExtractor ?? null;
        const emailAttachments = adapter.bankEmailAttachments ??
    ...
    router.post('/api/bank-import/preview-multiformat', async (req, res) => {
        const body = (req.body ?? {});
        res.json(await previewMultiformat(getMultiformatParser(), String(body.content ?? ''), body.format ?? null));
    });
    router.post('/api/bank-import/validate-csv', async (req, res) => {
        const body = (req.body ?? {});
        res.json(await validateCsv(getMultiformatParser(), String(body.content ?? '')));
    });
    router.get('/api/bank-import/statement-review/:import_id', async (req, res) => {
        const appDb = getAppDb(req, res);
        if (!appDb)
            return;
        res.json(await getStatementReview(appDb, Number(req.params.import_id)));
    });
    // ---------------------------------------------------------------
    // import-from-email + import-from-statement
    // ---------------------------------------------------------------
    const handleImportFromEmail = async (req, res) => {
        const operaDb = getOperaDb(req, res);
        if (!operaDb)
            return;
        const appDb = getAppDb(req, res);
        ...
        if (!extractor || !adapter.bankEmailAttachments) {
            res.status(503).json({
                success: false,
                error: 'PDF extractor (or ctx.llm) and email-attachment provider must both be configured.',
            });
            return;
        }
        const executor = adapter.bankImportExecutor ?? bankImportPostingExecutor;
        ...
        res.json(await importBankStatementFromEmail(operaDb, appDb, adapter.bankEmailAttachments, extractor, executor, lock, overlap, input));
    };
    router.post('/api/bank-import/import-from-email', handleImportFromEmail);
    // Composite alias used by the legacy UI
    router.post('/api/reconcile/bank/:bank_code/import-from-statement', async (req, res) => {
        // Forward to import-from-pdf with the bank_code from the path
        req.url = '/api/bank-import/import-from-pdf';
        req.query.bank_code = String(req.params.bank_code ?? '');
        router.handle(req, res, () => undefined);
    });
    return router;
}
