...
export function classifyExtractionError(err) {
    ...
    if (lower.includes('resource_exhausted') ||
        ...
        return {
            kind: 'quota',
            transient: false,
            message: "Gemini quota exhausted for today. Wait until your billing " +
    ...
    if (lower.includes('400') || lower.includes('invalid_argument')) {
        return {
            kind: 'bad_request',
            transient: false,
            message: 'Gemini rejected the request (malformed PDF, oversize file, ' +
...
export async function withRetry(op, opts = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            opts.logger?.warn?.(`${opts.label ?? 'retry'}: ${cls.kind} on attempt ${attempt}/${maxAttempts}, waiting ${delay}ms`);
...
export function _resetGeminiBreakerForTesting() {
...
//# sourceMappingURL=extraction-error.js.map