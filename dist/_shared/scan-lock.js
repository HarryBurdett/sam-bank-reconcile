...
export class ScanInProgressError extends Error {
    key;
    elapsedMs;
    constructor(key, elapsedMs) {
        super(`Scan already in progress for "${key}" — started ${Math.round(elapsedMs / 1000)}s ago. Wait for it to complete, or retry in a moment.`);
        this.key = key;
 ...
 */
export async function withScanLock(key, work) {
...
export function _resetScanLocksForTesting() {
    inFlight.clear();
}
/** Diagnostic — current lock holders and ages. */
export function getActiveScanLocks() {
    const now = Date.now();
